...
(async () => {
  ...
  if (!process.env.PRODUCT_TYPE_KEY) {
    throw new Error("PRODUCT_TYPE_KEY is not defined");
  }
  ...
  for (let linkNumber = 0; linkNumber < 1; linkNumber += 1) {
    ...
    let ProductImagesCard = await itemPage.evaluate(
      (linkNumber, IMAGES_STORAGE, PRODUCT_TYPE_KEY) => {
        try {
          let titleDetails =
    ...
    let { productImages, detailsImages } = await itemPage.evaluate((P) => {
    ...
    let ProductVariants = await itemPage.evaluate(
      (linkNumber, PRODUCT_TYPE_KEY) => {
        try {
              ...
              if (isMaster) {
                ProductCard.push(
                  ...
                  },
                  {
                    ...
                    productTypeKeyValue: `${PRODUCT_TYPE_KEY}`,
                  },
                  {
